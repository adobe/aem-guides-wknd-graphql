...
/**
 * Calls the 'wknd-shared/adventure-by-path' persisted query with `path` parameter.
 *
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (response.err) {
        // Capture errors from the HTTP request
        setErrors(response.err);
      } else if (response.data?.adventureByPath?.item) {
        // Set the Adventure data after data validation
        setAdventure(response.data.adventureByPath.item);
        //setReferences(response.data.adventureByPath._references);
      } else {
        // Set an error if no Adventure could be found
        setErrors(`Cannot find Adventure with path: ${path}`);
      }
